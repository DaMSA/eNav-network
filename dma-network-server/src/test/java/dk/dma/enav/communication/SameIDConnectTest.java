...
public class SameIDConnectTest extends AbstractNetworkTest {
    ...
    public void twoConnect() throws Exception {
        ...
        MaritimeNetworkConnection pc2 = newClient(ID1);
    ...
    public void manyConnect() throws Exception {
        ...
        final Set<Future<MaritimeNetworkConnection>> s = Collections
                .newSetFromMap(new ConcurrentHashMap<Future<MaritimeNetworkConnection>, Boolean>());
        for (int i = 0; i < 100; i++) {
            s.add(e.submit(new Callable<MaritimeNetworkConnection>() {
        ...
        Set<MaritimeNetworkConnection> con = new HashSet<>();
        for (Future<MaritimeNetworkConnection> f : s) {
            try {
                con.add(f.get());
        ...
        for (int i = 0; i < 100; i++) {
            if (si.getNumberOfConnections() == 1) {
                int connectCount = 0;
                for (MaritimeNetworkConnection pc : con) {
